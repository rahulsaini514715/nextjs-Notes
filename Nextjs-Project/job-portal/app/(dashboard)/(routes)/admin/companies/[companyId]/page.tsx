...
import CompanyLogo from "./logo-form";
import CompanySocialContactsForm from "./social-contacts-form";
import CompanyCoverImage from "./cover-image-form";
import CompanyOverviewForm from "./company-overview";
import WhyJoinUsForm from "./why-join-us-form";
const CompanyEditPage = async ({ params }: { params: Promise<{ companyId: string }> }) => {
   const { companyId } = await params;
  
    //verify the mongodb ID
    const validObjectIdRegex = /^[0-9a-fA-F]{24}$/ ;
  
    if(!validObjectIdRegex.test((await params).companyId)){
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16">
          <div>
             ...
             <div className="flex items-center gap-x-2">
               ...
               <h2 className="text-xl text-neautral-700">Customize your Company</h2>
             </div>
