...
const CompanyOverviewForm = ({ initialData, companyId }: CompanyOverviewFormProps) => {
  ...
  const handlePromptGeneration = async () => {
    ...
    try {
      setIsPrompting(true);
      const customPrompt = `Generate an overview content about ${rollname}
      Include information about its history, purpose, features, user base,
      and impact on the industry. Focus on providing a comprehensive yet
      concise summary suitable for readers unfamiliar with the platform `;
    ...
    <div className="mt-6 border bg-neutral-100 rounded-md p-4">
      ...
      {isEditing && (
        <>
          ...
          {aiValue && (
            <div className="w-full h-96 max-h-96 rounded-md bg-white overflow-y-scroll 
            p-3 relative mt-4 text-muted-foreground
            ">
                {aiValue}
